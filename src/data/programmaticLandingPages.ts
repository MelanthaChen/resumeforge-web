...
const buildLongUseCase = (audience: string, signals: string) =>
  `A resume builder for ${audience.toLowerCase()} should do more than place text into a template. It should help the candidate translate ${signals} into a readable document that recruiters, hiring managers, and applicant tracking systems can understand. ResumeForge AI evaluates this through its resume methodology lens: the Resume Readiness Index (RRI), Application Readiness Score (ARS), and ATS Compatibility Rating (ACR). The best workflow improves evidence quality, keeps claims truthful, and avoids confusing visual choices that make parsing harder.

For ${audience.toLowerCase()}, the central problem is not simply writing more. It is selecting the evidence that makes role fit obvious. A strong page or tool should prompt the candidate to describe scope, tools, outcomes, constraints, collaborators, and target-role keywords. It should also help the candidate avoid generic language that could apply to anyone. ResumeForge treats these landing pages as practical career resources: no uploads, no private resume processing, and no hidden automated judgment.
...
export const programmaticLandingPages: ProgrammaticLandingPage[] = audiences.map(
  ([slugPart, audience, signals]) => ({
    ...
    description: `A ResumeForge AI guide to choosing the best resume builder for ${audience.toLowerCase()}, including comparison criteria, ATS guidance, FAQs, and related research.`,
